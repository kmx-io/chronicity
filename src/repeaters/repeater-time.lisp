...
(defmethod initialize-instance :after ((repeater repeater-time) &key type &allow-other-keys)
          ...
          (cond
            ...
            ((>= ts-size 6)
             (let* ((hours (parse-integer ts :end 2))
                    ...
                    (secs (parse-number ts :start 4))
                    (isecs (truncate secs))
                    (nsec (round (mod secs 1) (expt 10 -9)))
                    (ambiguousp (and (scan ":" type)
                                     (/= (parse-integer ts :end 1) 0) ; Redundant?
                                     (<= hours 12))))
               (if (= hours 12)
                   (make-tick (make-time 0 minutes isecs nsec) ambiguousp)
                   (make-tick (make-time hours minutes isecs nsec) ambiguousp))))))))
